...
class Network:
	...
	def __init__(self, branch_type: str, message_length, noise_layers_R, noise_layers_F, device, batch_size, lr, beta1, attention_encoder, attention_decoder, weight):
		# device
		self.device = device
		...
		# network
		# encoder-decoder: select ST or FD branch
		if branch_type == "ST":
			from .ST_EncoderDecoder import ST_EncoderDecoder
			self.encoder_decoder = ST_EncoderDecoder(message_length, noise_layers_R, noise_layers_F, attention_encoder, attention_decoder).to(device)
		elif branch_type == "FD":
			from .FD_EncoderDecoder import FD_EncoderDecoder
			self.encoder_decoder = FD_EncoderDecoder(message_length, noise_layers_R, noise_layers_F, attention_encoder, attention_decoder).to(device)
		else:
			raise ValueError(f"Unknown branch type: {branch_type}. Choose 'ST' or 'FD'.")
	...
	def train_ST(self, images: torch.Tensor, messages: torch.Tensor, masks: torch.Tensor):
		self.encoder_decoder.train()
		self.discriminator.train()
  
		with torch.enable_grad():
			images, messages, masks = images.to(self.device), messages.to(self.device), masks.to(self.device)
			encoded_images, noised_images, decoded_messages_C = self.encoder_decoder(images, messages, masks)
	...
	def train_FD(self, images: torch.Tensor, messages: torch.Tensor, masks: torch.Tensor):
		self.encoder_decoder.train()
		self.discriminator.train()
  
		with torch.enable_grad():
			images, messages, masks = images.to(self.device), messages.to(self.device), masks.to(self.device)
			encoded_images, noised_images, decoded_messages_R, decoded_messages_F = self.encoder_decoder(images, messages, masks)
	...
	def validation_ST(self, images: torch.Tensor, messages: torch.Tensor, masks: torch.Tensor):
		...
		with torch.no_grad():
			...
			g_loss_on_decoder_C = self.criterion_MSE(decoded_messages_C, messages)
			g_loss = self.encoder_weight * g_loss_on_encoder_LPIPS + self.decoder_weight_C * g_loss_on_decoder_C
	...
	def validation_FD(self, images: torch.Tensor, messages: torch.Tensor, masks: torch.Tensor):
		self.encoder_decoder.eval()
		self.encoder_decoder.module.noise.train()
		self.discriminator.eval()
		...
		with torch.no_grad():
			...
			g_loss_on_decoder_R = self.criterion_MSE(decoded_messages_R, messages)
			g_loss_on_decoder_F = self.criterion_MSE(decoded_messages_F, torch.zeros_like(messages))
			g_loss = self.encoder_weight * g_loss_on_encoder_LPIPS + \
					self.decoder_weight_R * g_loss_on_decoder_R + \
					self.decoder_weight_F * g_loss_on_decoder_F
